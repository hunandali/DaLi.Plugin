...
'			Dim url = AutoHelper.GetVarString(Me.Url, data).EmptyValue(setting.OPENAI_URL)
'			Dim apiKey = AutoHelper.GetVarString(Me.ApiKey, data).EmptyValue(setting.OPENAI_KEY)
'			Dim model = AutoHelper.GetVarString(Me.Model, data).EmptyValue(setting.OPENAI_MODEL)
...
'			'Dim ai As New AI.OpenAI.Chat(url, apiKey, model, Opts) With {.system = system}
'			'Dim res = ai.Process(content)
'			'message.SetSuccess(res.Success, res.Message.Content)
'			'Return If(res.Success, res.Message.Content, "")

'			Dim Opts = Options.FormatAction(Function(x) AutoHelper.GetVar(x, data)).ToJson.ToJsonObject(Of ChatOptions)
'			Dim ai As New AIChatProvider(Utils.AI.Model.AIClientEnum.OPENAI, url, apiKey, model) With {.SystemPrompt = system}
'			Dim chat = ai.Chat(content, Nothing, Opts)
