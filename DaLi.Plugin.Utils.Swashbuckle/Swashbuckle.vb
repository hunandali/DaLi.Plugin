	...
	Public Overrides Sub AddServices(services As IServiceCollection)
		If DocumentPath.NotEmpty Then
			...
			Dim Action = Sub(opt As SwaggerGenOptions)
							 ...
							 Dim XMLs = IO.Directory.GetFiles(PathHelper.Root, "*.xml", IO.SearchOption.AllDirectories)
							 If XMLs.NotEmpty Then
								 ' 所有程序集名称
								 Dim names = ControllerAssembies.Select(Function(x) x.Name).ToList

								 ' 分析所有包含程序集名称的项目，重复仅保留一个，最后加入文档
								 XMLs.Select(Function(x) New With {.Name = IO.Path.GetFileNameWithoutExtension(x), .Path = x}).
													Distinct(Function(x) x.Name).
													Where(Function(x) names.Contains(x.Name)).
													ToList.
													ForEach(Sub(x) opt.IncludeXmlComments(x.Path))
							 End If
						 End Sub
